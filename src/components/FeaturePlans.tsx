...
function FeaturePlans() {
  ...
  const payNow = async () => {
    ...
    const options = {
      ...
      handler: function (response: RazorpayResponse) { // Add type here
    ...
    razor.on("payment.failed", function (response: RazorpayErrorResponse) { // Add type here
          ...
          {featuredPlans.map((membership_plans: Plan) => (
            <div key={membership_plans.type} className='flex justify-center text-white'>
              <BackgroundGradient
                ...
                <div className="p-4 sm:p-6 relative flex-col items-center text-center flex-grow">Take membership for best buildup your fitness or health</div>
              </BackgroundGradient>
            </div>
          ))}
        </div>
